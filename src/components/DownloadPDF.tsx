...
/**
 * Télécharge l'histoire au format PDF via html2pdf.js.
 */
export default function DownloadPDF({ story }: DownloadPDFProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      container.style.cssText = 'width:800px;background:#0f172a;margin:0;padding:0;font-family:Georgia,serif;color:#e2e8f0;';
    ...
    <>
      ...
      >
        {loading ? (
          ...
          <><Check className="w-4 h-4" /> PDF prêt !</>
        ) : (
...
function getPDFHTML(story: Story): string {
  ...
  pagesHTML += `
    <div class="pdf-p" style="background:linear-gradient(135deg,#1e1b4b 0%,#0f172a 50%,#2e1065 100%);border:3px solid #a855f7;justify-content:center;">
      <div class="pdf-title">${esc(story.title)}</div>
      <div class="pdf-subtitle">Une histoire illustrée pour ${esc(story.heroName || 'toi')}</div>
  ...
  story.pages.forEach((p, i) => {
    ...
    pagesHTML += `
      <div class="pdf-p" style="background:${bg}">
        ...
        <div class="pdf-text">${esc(p.text)}</div>
        <div class="pdf-meta" style="color:#475569;margin-top:16px;">${esc(p.descriptionVisuelle || '')}</div>
      </div>`;
  });
